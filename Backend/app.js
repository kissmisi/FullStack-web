...
app.post('/kosarba', (req, res) => {
    dbconn()
    con.query(`INSERT INTO kosar VALUES (NULL, ${req.body.klimaId}, (SELECT id FROM felhasznalok WHERE id=${req.body.userId}), (SELECT Klimaberendezes FROM klimaberendezesek WHERE id=${req.body.klimaId}), (SELECT Ar FROM klimaberendezesek WHERE id=${req.body.klimaId}), 1, (SELECT Keszlet FROM klimaberendezesek WHERE id=${req.body.klimaId}))`, function (err, rows) {
        if (err) throw err
