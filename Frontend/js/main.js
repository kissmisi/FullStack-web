function klimaberendezesek3() {
  let url = "http://localhost:3000/klimaberendezesek";
  fetch(url)
    .then((x) => x.json())
    .then((y) => megjelenit(y));
...
function kosarba(btn) {
  ...
  fetch("http://localhost:3000/kosar", fetchO)
    .then(x => x.json())
    .then(y => vizsgal(y))
    ...
    function vizsgal(adatok) {
      ...
      if (adatok.length != 0) {
        for (var sor of adatok) {
          ...
          if (sor.klimaberendezes_id == id) {
            ...
            fetch("http://localhost:3000/kosarUpdate", fetchOp)
              .then((x) => x.json())
              .then((y) => alert(y));
      ...
      let url = "http://localhost:3000/kosarba";
...
function klimaOldal(btn) {
  ...
  let url = "http://localhost:3000/klimaberendezesek_2"
      ...
      oldal += `
        <div class="row container">
          ...
          <div class="m-0 p-0 mt-3 col-md-6">
            <img src="http://localhost:3000/${sor.kep}" alt="${sor.kep}"
