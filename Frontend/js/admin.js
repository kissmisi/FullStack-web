...
function klimaPost() {
    ...
    if (document.getElementById('klimaberendezes').value!="" && document.getElementById('teljesitmeny').value!="" && document.getElementById('klimaAr').value!="" && document.getElementById('keszlet').value!="") {
        let url="http://localhost:3000/klimaPost"
...
function klimaberendezesek2() {
    let url = "http://localhost:3000/klimaberendezesek3";
    fetch(url)
      .then((x) => x.json())
      .then((y) => megjelenit(y));
...
function klimaEdit(btn) {
  ...
  fetch("http://localhost:3000/klimaberendezesek_2",{
    method: "POST",
    body: JSON.stringify(bemenet),
    headers: { "Content-type": "application/json; charset=UTF-8",
                "Access-Control-Allow-Origin" : "*" }
...
function klimaModosit(){
  let url="http://localhost:3000/klimaEdit"
...
function felhasznalok() {
    let url = "http://localhost:3000/felhasznalokGet";
    fetch(url)
      .then((x) => x.json())
      .then((y) => megjelenit(y));
...
function userDelete(btn) {
  ...
  if (confirmAction) {
    ...
    let url="http://localhost:3000/felhasznalokDelete"
...
function klimaDelete(btn) {
  ...
  if (confirmAction) {
    let url="http://localhost:3000/klimaDelete"
    let bemenet={
      id: id,
    }
...
function rendelesek(){
  let url="http://localhost:3000/rendelesek"
...
function rendelesElfogad(btn){
  ...
  let url="http://localhost:3000/rendelesElfogad"
...
function rendelesDelete(btn){
  ...
  if(confirmAction){
    let url="http://localhost:3000/rendelesDelete"
...
function userEdit(btn){
  ...
  fetch("http://localhost:3000/felhasznaloEdit", {
    method: "POST",
    body: JSON.stringify(bemenet),
    headers: { "Content-type": "application/json; charset=UTF-8",
                "Access-Control-Allow-Origin" : "*" }
...
function userModosit() {
  let url="http://localhost:3000/userEdit"
