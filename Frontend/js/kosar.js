function Kosar() {
    ...
    function megjelenit(adatok) {
        ...
        for (var sor of adatok) {
            table1+=`
                <tr style="width: 100%">
                    ...
                    <td><button class="btn btn-success" onclick="mennyisegMinusz(this)"><i class="fa-solid fa-minus"></i></button></td>
                    <td style="text-align: center;"><input type="number" value="${sor.mennyiseg}" min="1" max="${sor.max}" onchange="mennyiseg(this)" style="width: 60%; margin-top: 9%; text-align: center"></td>
                    <td><button class="btn btn-success" onclick="mennyisegPlusz(this)"><i class="fa-solid fa-plus"></i></button></td>
        ...
        let url="http://localhost:3000/kosar2"
...
function mennyisegMinusz(inpt) {
    let tr=inpt.parentElement.parentElement
    let mennyiseg=tr.querySelector('input').value
    ...
    let url="http://localhost:3000/kosarUpdate2"
    ...
    let fetchO = {
      ...
      fetch(url, fetchO)
        .then(x => x.json())
        .then(y=>{
            fetch("http://localhost:3000/kosar2", fetchOp)
            .then(a=>a.json())
            .then(b=>megjelenit(b))
        })
...
function kosarUrit() {
    ...
    if(confirmAction) {
        fetch("http://localhost:3000/kosarUrit", fetchO)
        .then(x=>x.text())
        .then(y=>{
            console.log(y)
...
function kosarDelete(btn) {
    ...
    let url="http://localhost:3000/kosarDelete"
